...
<div id="list-score" class="content scaffold-list" role="main">
    <h1>robot design score aggregation (by contestant)</h1>
    ...
    <table class="list">
        ...
        <g:each in="${aggregates}" status="i" var="scoreInstance">
            <tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
                ...
                <td>${fieldValue(bean: scoreInstance, field: "aggTotal")} (${((fieldValue(bean: scoreInstance, field: "aggTotal").toFloat() / 4.toFloat())*100).round(3).toString()}%) ${com.charlesread.RoomAverages.get(ScoreTechnical.findByContestant(scoreInstance.contestant)?.judge?.roomId)}</td>
