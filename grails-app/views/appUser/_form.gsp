...
<div class="fieldcontain">
    <label for="roles">
        <g:message code="appUser.authority.label" default="Roles" />
    </label>
    <select multiple name="roles">
        <g:each in="${com.charlesread.AppRole.list()}" var="appRoleInstance">
            <option <g:if test="${params.action.equals('create')}"></g:if><g:elseif test="${com.charlesread.AppUserAppRole.findByAppUserAndAppRole(appUserInstance,appRoleInstance)}">selected</g:elseif> value="${appRoleInstance.id}">
                ${appRoleInstance}
            </option>
        </g:each>
    </select>
